...
export const filterCharacters = (query, charStatus, charGender, value) => async dispatch => {
    try {
        dispatch({
            type: "LIST_LOADING"
        })
        //QUERY BY ALL FILTERS
        const res = await axios.get(`https://rickandmortyapi.com/api/character/?name=${query}&status=${charStatus}&gender=${charGender}&page=${value}`)
        dispatch({
...
export const getCharacter = (charID) => async dispatch => {
    try {
        ...
        const res = await axios.get(`https://rickandmortyapi.com/api/character/${charID}`)
        dispatch({
